...
<?php /*?>		<div class="post" id="post-<?php the_ID(); ?>">
	...
	<?php endwhile; endif; ?>
	<?php $loop = new WP_Query('post_type=portfolio&orderby=menu_order&order=ASC&posts_per_page=999'); ?>
    ...
    <?php
		...
		?>
		<div class="post <?php if ($terms != NULL) { echo "folio"; } ?>" id="post-<?php the_ID(); ?>">
            <div class="entryInfo">
            <?php
                if ($theSlug === "video") { if(get_field('the_video')) $theLarge[0] = get_field('the_video');
				else $theLarge[0] = "";
					$theLarge[1] = 480;
					$theLarge[2] = 380;
                ...
                elseif ($theSlug === "audio") {
                    if(get_field('the_audio')) $theLarge[0] = get_field('the_audio');
					else $theLarge[0] = "";
					$theLarge[1] = 480;
					$theLarge[2] = 27; ?>
                    <textarea class="theImageURL">
                ...
                elseif ($theSlug === "embed") {
                    if(get_field('the_embed')) $theLarge[0] = get_field('the_embed');
                    else $theLarge[0] = "";
                    $theLarge[1] = 480;
                    $theLarge[2] = 380;?>
                    <textarea class="theImageURL">
                ...
                elseif ($theSlug === "iframe") {
                    if(get_field('the_iframe')) $theLarge[0] = get_field('the_iframe');
                    else $theLarge[0] = "";
                    $theLarge[1] = 480;
                    $theLarge[2] = 380;?>
                    <textarea class="theImageURL">
                ...
                elseif ($theSlug === "print") {
					...
					{$theLarge = wp_get_attachment_image_src( get_field('the_image'), 'medium' );
					}
					else {$theLarge[0] = "";$theLarge[1] = "";$theLarge[2] = ""; } ?>
					<textarea class="theImageURL" height="<?php echo $theLarge[2]; ?>" width="<?php echo $theLarge[1]; ?>"><?php echo $theLarge[0]; ?></textarea>
                <?php } ?>
            </div><!--/entryInfo-->
			<div class="entry">
            	<h3><?php if ($theSlug != "") { ?><span class="<?php echo $theSlug." "; ?>"></span><?php } ?><?php the_title(); ?></h3>
				<?php the_excerpt(); ?>
			</div><!--/entry-->
		</div><!--/post-->
    <?php endwhile; ?>
