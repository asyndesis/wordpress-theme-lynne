...
function init() {
  setStage();
  registerNav();
  $("body").animate({ margin: 0 }, 1000, function () {
    scrollOpen(300);
  });
}
/* 2.1.1 setStage
  ...
  $("#scrollContent")
    ...
    .each(function () {
      ...
      if ($("#scrollContent").hasClass("contact")) {
        animateHover($(this).find("input"), 0.4);
        animateHover($(this).find("textarea"), 0.4);
      } else {
        animateHover($(this), 0);
      }
      if (
        $("#scrollContent").hasClass("portfolio") &&
        $(this).hasClass("folio")
      ) {
        lightBoxer($(this));
        $(this).hover(
          function () {
            ...
            } else if (s.hasClass("video")) {
              t = "CLICK TO VIEW VIDEO";
            } else if (s.hasClass("print")) {
              t = "CLICK TO VIEW PHOTO";
            } else if (s.hasClass("embed")) {
              t = "CLICK TO VIEW VIDEO";
            } else if (s.hasClass("iframe")) {
              t = "CLICK TO VIEW VIDEO";
            } else {
              return;
            }
  ...
  if (detectIE() == "-1") {
    ...
    o.hover(
      function () {
        $(this)
          .stop(false, true)
          .animate({ opacity: hoverOpacity }, defaultSpeed);
      },
      function () {
        if (!$(this).is(":focus"))
          $(this)
            .stop(false, true)
            .animate({ opacity: defaultOpacity - theOffset }, defaultSpeed);
      }
    );
    o.focus(function () {
      $(this)
...
function scrollOpen(theDelay) {
  ...
  $("#scrollMiddle").animate(
    { height: scrollHeight + 150 + "px" },
    defaultSpeed,
    function () {
...
function scrollClose(theUrl) {
  ...
  $("#thePen")
    .stop(false, true)
    .animate({ opacity: 0 }, defaultSpeed, function () {
      $("#scrollMiddle")
        .stop(false, true)
        .animate({ height: scrollHeight }, defaultSpeed, function () {
